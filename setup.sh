...
if test -f ~/.bashrc;
then
    echo "source $(find ~/go/pkg/mod/github.com/tomnomnom/ -name gf-completion.bash)" >> ~/.bashrc
    source ~/.bashrc
fi
...
then
    ...
    echo "source $(find ~/go/pkg/mod/github.com/tomnomnom/ -name gf-completion.zsh)" >> ~/.zshrc
    source ~/.zshrc
fi
